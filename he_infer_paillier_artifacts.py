...
# load features
Xte = np.load(FD/"test_features.npz")["X"]
yte = np.load(FD/"test_features.npz")["y"]

# load model/scaler (robust với Pipeline)
model_obj = load(MD/"model_best.joblib")
scaler = None
clf = model_obj
if isinstance(model_obj, Pipeline):
    ...
    if clf is None:
        raise ValueError("Không tìm thấy step 'clf' trong Pipeline.")
# nếu chưa có scaler, thử lấy từ file riêng
if scaler is None:
    try:
        scaler = load(MD/"scaler_best.joblib")
    except:  # nếu không có thì coi như không scale
        class _NoScaler:
            def transform(self, X): return X
        scaler = _NoScaler()

# đọc ngưỡng tối ưu (xác suất) → đổi sang logit
cfg = json.loads((MD/"best_config.json").read_text(encoding="utf-8"))
thr_p = float(cfg.get("thr_opt", 0.5))
thr_s = logit(thr_p)

# lấy trọng số tuyến tính
if not hasattr(clf, "coef_") or not hasattr(clf, "intercept_"):
    raise ValueError("Mô hình không phải LR/LDA tuyến tính (không có coef_/intercept_).")
...
print(f"[PLAINTEXT] N={len(Xt)}  AUC={auc_plain:.4f}  Acc@opt={acc_plain:.4f}")
...
print(f"[HE-Paillier] N={len(Xt)}  AUC={auc_he:.4f}  Acc@opt={acc_he:.4f}  time={t1-t0:.2f}s  ~{(t1-t0)/len(Xt):.3f}s/sample")
