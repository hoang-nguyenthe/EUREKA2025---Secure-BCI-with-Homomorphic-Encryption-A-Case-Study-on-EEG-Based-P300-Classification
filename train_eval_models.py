...
def main():
    ...
    for subject in subject_dirs:
        ...
        features_file = IN / f"{subject}_test_features.npz"
        if not features_file.exists():
            print(f"Không tìm thấy tệp {features_file}, bỏ qua {subject}.")
            continue

        tr = np.load(IN / f"{subject}_train_features.npz")
        Xtr, ytr = tr["X"], tr["y"]
        te = np.load(features_file)
        Xte, yte = te["X"], te["y"]

        # Thực hiện Stratified KFold
        ...
        for clf, params, name in build_estimators(linear_only=args.linear_only):
            ...
            grid = GridSearchCV(pipe, param_grid=(params or {}), scoring=scoring, refit="roc_auc", cv=cv, n_jobs=-1, verbose=0)
            grid.fit(Xtr, ytr)
            y_score_tr = (grid.best_estimator_.predict_proba(Xtr)[:,1] if hasattr(grid.best_estimator_, "predict_proba") else grid.best_estimator_.decision_function(Xtr))
            record = {
        ...
        pd.DataFrame(rows).to_csv(OUT / f"{subject}_results_cv.csv", index=False)
        print(f"=== CV SUMMARY for {subject} ===")
        ...
        if hasattr(best_est, "predict_proba"):
            ...
            s_val = best_est.decision_function(Xtr[val_idx])
            s = (s_val - s_val.min()) / (s_val.max() - s_val.min() + 1e-9)
            p_val = s
        ...
        if hasattr(best_est, "predict_proba"):
            ...
            s_te = best_est.decision_function(Xte)
            s = (s_te - s_te.min()) / (s_te.max() - s_te.min() + 1e-9)
            p_te = s
        ...
        dump(best_est, OUT / f"{subject}_model_best.joblib")
        dump(scaler, OUT / f"{subject}_scaler_best.joblib")
        cfg = {"winner": best_name, "thr_opt": thr_opt}
        (OUT / f"{subject}_best_config.json").write_text(json.dumps(cfg, indent=2), encoding="utf-8")
        print(f"Saved: {OUT / f'{subject}_model_best.joblib'} {OUT / f'{subject}_scaler_best.joblib'} {OUT / f'{subject}_best_config.json'}")
